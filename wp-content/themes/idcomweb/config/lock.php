...
if ( ! class_exists('IDCOM_LockPages') ) {

	class IDCOM_LockPages {
		...
		*/
		var $localization_domain = "idcomcrea";
		/**
		...
		*/
		function add_hooks() {
			...
			add_filter( 'post_class', array( &$this, 'post_class' ), 0, 3 );

		}
		function body_class( $classes){
			$post_id=get_the_ID();
			if(!current_user_can( $this->options[$this->prefix.'capability'] )&&$this->is_page_locked( $post_id )){
				$classes[]="idcom-page-locked";
			}
			return $classes;
		}
		...
		*/
		function pages_list_col( $cols, $post_type = 'page' ) {
			...
			if (
				...
				$cols=array('cb'=>$cols['cb'],'titre2'=>"Titre",'date'=>$cols['date']);
			}
			return $cols;
		}
		...
		*/
		function pages_list_col_value( $column_name, $id ) {
			...
			switch($column_name){
				case "post-locked":echo $this->is_page_locked( $id ) ? '<span class="dashicons dashicons-lock"></span>' : '&nbsp;';break;
				case "titre2":
					if(! current_user_can( $this->options[$this->prefix.'capability'] )&&$this->is_page_locked( $id )){ 
						echo '<span class="idcom_verrou"><span class="dashicons dashicons-lock"></span> <strong> '.get_the_title($id).'</strong> <em>Cette page a été verrouillée par l\'administrateur</em></span>';
					}else{ 
						if(get_option('page_for_posts')==$id){
							echo '<a href="'.get_edit_post_link($id).'"><span class="
dashicons 
dashicons-megaphone"></span> <strong>'.get_the_title($id).'</strong></a> <em>Page des articles</em>';
						}elseif(get_option('page_on_front')==$id){
							echo '<a href="'.get_edit_post_link($id).'"><span class="
dashicons dashicons-admin-home"></span> <strong>'.get_the_title($id).'</strong></a> <em>Page d\'accueil</em>';
						}else{
							echo '<a href="'.get_edit_post_link($id).'">'.get_the_title($id).'</a>';
						}
					} 
					echo "<style>.level-1 .titre2{ padding-left:35px;}.level-2 .titre2{ padding-left:70px;}.level-3 .titre2{ padding-left:105px;}.level-4 .titre2{ padding-left:140px;}</style>";
					break;
			}
		}
		...
		*/
		...
		function create_meta_box( $post_type, $post ) {
			if (
				...
				add_meta_box( $this->prefix.'_meta-box', '<i class="dashicons dashicons-lock"></i> Verrouillage', array( &$this, 'output_meta_box' ), $post_type, 'side', 'high' );
			}
		}
		...
		function output_meta_box() {
			if ( current_user_can( $this->options[$this->prefix.'capability'] ) ) {
				...
				<input type="hidden" name="<?php echo esc_attr( $this->prefix ); ?>meta_nonce" value="<?php echo wp_create_nonce(wp_basename(__FILE__)); ?>" />
				<label for="<?php echo esc_attr( $this->prefix ); ?>locked">
					...
					<?php
					if ( get_post_type() == 'page' ) {
						_e( 'Verrouiller cette page', $this->localization_domain );
					} else {
						_e( 'Verrouiller cet élément', $this->localization_domain );
					}
					?>
				</label>
		...
		*/
		function load_js_css() {
			wp_enqueue_script( $this->prefix . '_js', get_stylesheet_directory_uri(). '/config/lock-pages.js', array( 'jquery' ), '3.0' );
			wp_enqueue_style( $this->prefix . '_css', get_stylesheet_directory_uri(). '/config/lock-pages.css', array(), '3.0' );
		}
		...
		*/
		function admin_menu_link() {
			...
			//add_submenu_page( 'admin_idcomcrea','Verrouillage Pages', 'Verrouillage Pages', 'update_core', 'admin.php?page=verrouillage_pages', array( &$this, 'admin_options_page' ) );
		...
		function admin_options_page() {
			...
			*/
			...
			<div class="">
				...
				<h2 class="mdl-layout-title">Verrouillage des pages/articles</h2>
				...
				<form method="post" id="SLT_LockPages_options">
					...
					<table width="100%" cellspacing="2" cellpadding="5" class="mdl-data-table mdl-js-data-table mdl-shadow--2dp" style="width: 90%">
						<tbody>
						<tr valign="top">
							<th width="33%" scope="row"><label for="<?php echo esc_attr( $this->prefix ); ?>capability">Droit minimum pour modifier les pages</label></th>
							<td class="mdl-data-table__cell--non-numeric"><input name="<?php echo esc_attr( $this->prefix ); ?>capability" type="text" id="<?php echo esc_attr( $this->prefix ); ?>capability" size="45" value="<?php echo esc_attr( $this->options[$this->prefix.'capability'] ); ?>"/></td>
						</tr>

						<tr valign="top">
							<th width="33%" scope="row">Type de verrouillage</th>
							<td class="mdl-data-table__cell--non-numeric">
								<input name="<?php echo esc_attr( $this->prefix ); ?>scope" type="radio" id="<?php echo esc_attr( $this->prefix ); ?>scope_locked" value="locked"<?php if ( $this->options[$this->prefix.'scope']=="locked" ) echo ' checked="checked"'; ?> /> <label for="<?php echo esc_attr( $this->prefix ); ?>scope_locked">Verrouiller certaines pages</label><br />
								<input name="<?php echo esc_attr( $this->prefix ); ?>scope" type="radio" id="<?php echo esc_attr( $this->prefix ); ?>scope_locked" value="all"<?php if ( $this->options[$this->prefix.'scope']=="all" ) echo ' checked="checked"'; ?> /> <label for="<?php echo esc_attr( $this->prefix ); ?>scope_all">Verrouiller toutes les pages</label>
							</td>
						</tr>

						<tr valign="top">
							<th width="33%" scope="row">Type de contenu à vérrouiller</th>
							<td class="mdl-data-table__cell--non-numeric">
								<?php
					...
					<p class="submit"><input type="submit" value="Enregistrer les paramètres"  class="mdl-button mdl-js-button mdl-button--raised"  name="SLT_LockPages_save" /></p>
...
if ( class_exists('IDCOM_LockPages') ) {
	$SLT_LockPages_var = new IDCOM_LockPages();
